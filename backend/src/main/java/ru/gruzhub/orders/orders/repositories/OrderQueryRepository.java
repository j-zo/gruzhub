...
public class OrderQueryRepository {
     ...
     * customer ID, driver ID, auto ID, user ID, region IDs, statuses, and a limit.
     *
     ...
     * @param autoId     the ID of the auto (optional)
    ...
    public List<Order> findOrders(Long masterId,
        ...
        // Filter by auto ID if provided (join with autos)
        if (autoId != null) {
            Join<Order, Auto> autoJoin = orderRoot.join("autos", JoinType.INNER);
            predicates.add(cb.equal(autoJoin.get("id"), autoId));
        }
